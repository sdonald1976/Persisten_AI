...
| **R-04** | ~~High~~ **Resolved, and partly wrong as filed** | Test safety net | **Correction (2026-08-26):** this finding claimed no byte-level golden existed for CompactV2. One did, in `tools/Companion.PlanV3.Prototype/CorpusGoldenTests.cs` — a file the audit never opened, having counted `[Fact]` in the sibling `PlanV3Tests.cs` and stopped. It covers **804** plans through `FromV2 → Validate → TranslateToV2 → CompactV2`, ordinal-equal. The docs' further claim that all 804 match their frozen `plan2` strings is overstated: that half runs on the **289** rows that have one. CompactV3 and CompactV4 genuinely had none, and now do (`22f8b59`), along with prompt-render and EF-model goldens (`5d3be02`) | Was: no mechanical proof of byte stability. Now covered for plan/2, plan/3, plan/4, the rendered prompt, and the EF model | `tools/Companion.PlanV3.Prototype/Goldens/`, `tests/Companion.Tests/Goldens/` | High | done | Goldens are the tests; regenerate only via `tools/Companion.Goldens --accept` |
...
**Blocker - contract-layer mismatch between plan validation and plan serialization.**
Recorded 2026-08-26, found while writing the CompactV3 golden (`22f8b59`). Two corpus plans
pass `PlanV3Codec.Validate` and are then **refused** by `PlanV3Codec.CompactV3`, whose
coaching lint runs at serialization time and throws `InvalidOperationException`. Validation
success followed by serialization refusal means "valid" does not mean "renderable", and no
caller can currently tell the difference without catching an exception.

The refusal is pinned in `tools/Companion.PlanV3.Prototype/Goldens/compact-v3-manifest.txt`
as `refused-by-lint`, which is the correct interim treatment: it makes the behaviour visible
and would fail if a refactor quietly started accepting those plans. It is not a resolution.

**Must be resolved before the Plan/4 corpus freeze.** The decision is *where the coaching
lint belongs* - in plan validation, in source-contribution validation, or in a separately
named serialization-eligibility check that callers can consult without throwing. Two things
are explicitly not resolutions: silently excluding the affected plans from the corpus, and
weakening the lint so the corpus passes. Either would make the corpus agree with the code by
changing what is measured rather than what is true.
